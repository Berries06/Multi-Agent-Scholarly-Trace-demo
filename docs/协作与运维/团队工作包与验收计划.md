...
|---|---|---|---|---|
| WP1 语料、解析与标注 | 3 个领域各 30 篇元数据记录，19 篇证据卡参与建图；PlainText/PyPDF 解析；Docling 可选适配 | Docling；GROBID；冻结 PDF 盲测集 | 章节、表格、引文、页码/边界框和高质量金标准 | A |
...
| WP5 应用与复现实验 | React + FastAPI + PyQt6 统一产品、130 项测试、Track A 四组决策变体与六实验入口 | 统一实验 CLI；MLflow 可选跟踪；PyInstaller 桌面发布 | 数据冻结、实验复现、评审演示、失败恢复 | D 主责，全员集成 |
...
- React 页面和 PyQt6 桌面端调用同一个 FastAPI；
- `unittest` 做工程回归；
- JSON 保存知识库和输出；
