...
- `scripts/实验台/shared_evidence_decision_lab.py`：只在 `snowsong.top` 的 CPU 公网机运行；
- `scripts/实验台/gliner_entity_lab.py`：只在项目腾讯云 GN7 GPU 节点运行，由 GPU 主动建立反向 SSH 隧道。
...
.venv\Scripts\python.exe -m pip install pip==25.1.1 streamlit==1.60.0
$env:LAB_SHARED_PASSWORD="仅供本机验收的长随机密码"
.venv\Scripts\python.exe -m streamlit run scripts/实验台/shared_evidence_decision_lab.py --global.developmentMode false --server.address 127.0.0.1 --server.port 8501 --server.baseUrlPath AgentDemo/lab
```
...
scp -o "BindAddress=$bindAddress" scripts/实验台/shared_evidence_decision_lab.py deploy/labs/evidence.env.example deploy/systemd/yanhai-evidence-lab.service deploy/nginx/agentdemo-labs.locations.conf "${publicSsh}:/tmp/yanhai-labs-deploy/"
ssh -o "BindAddress=$bindAddress" $gpuSsh "install -d -m 0700 /tmp/yanhai-labs-deploy"
scp -o "BindAddress=$bindAddress" scripts/实验台/gliner_entity_lab.py deploy/labs/gliner.env.example deploy/labs/tunnel.env.example deploy/labs/tencent-gn7-env.json deploy/systemd/yanhai-gliner-lab.service deploy/systemd/yanhai-gliner-tunnel.service "${gpuSsh}:/tmp/yanhai-labs-deploy/"
```
