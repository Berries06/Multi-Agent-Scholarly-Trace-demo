...
没有原文证据、端点不存在、关系越过 schema 或裁判未通过的候选，不得进入正式图谱。
...
- 裁判：结合模型分数、批判项、证据数量与来源质量，输出 `accepted`、`needs_review` 或 `rejected`。

裁判阈值只在验证集上确定。多智能体方案的价值必须通过“去批判者/去裁判”的消融实验来证明，不能用角色名称代替效果证据。
...
主结果至少比较：规则基线、单模型抽取、单次 LLM、DeepKE/OneKE 或同类模型、本项目完整方法。消融至少删除 schema、全文上下文、实体融合、批判者、裁判和证据检查。所有方案使用同一冻结测试集，报告均值、标准差与统计显著性。
...
当前 8 篇种子文献的离线基线产出 11 个实体、14 条候选关系；6 条通过自动裁决，8 条弱共现关系进入复核，证据绑定完整率为 100%。它只证明“关系有本地引用 ID”的结构护栏生效，不证明引用语义正确；这是工程冒烟测试，不是论文性能结论。
