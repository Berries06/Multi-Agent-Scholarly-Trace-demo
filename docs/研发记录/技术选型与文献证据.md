...
落地：`src/yanhai/extraction.py`、`src/yanhai/graph_rag.py`、`docs/归档/配置/旧GraphRAG路线.json`。限制：当前社区算法是无外部依赖的连通分量基线，不是官方 Global/Local Search；正式实验需通过 BYOG 接入 Leiden 社区、community reports 和 embeddings。
