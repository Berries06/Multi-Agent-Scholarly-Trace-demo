...
| WP4 多智能体证据校验 | 6 个确定性 Agent；规则裁判 | Qwen2.5-7B-Instruct；SciFact verifier；Debate-Augmented RAG 思路 | 单次抽取 vs. 提出/批判/裁判消融、置信校准、成本 | D |
...
当前固定 6 个 Agent，其中与可信入图直接相关的是提出者、批判者和裁判。三者目前都是确定性规则：
