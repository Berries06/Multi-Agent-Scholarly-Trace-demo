...
面向跨学科科研全文，schema 约束的科学信息抽取与“命题生成—内部批判—质量准入”校验，能否在保持证据可追溯的前提下，提高知识图谱三元组的精确率、跨论文实体融合质量和下游研究发现效用？
...
|---|---|---|---|---|
| WP1 语料、解析与标注 | 8 篇元数据/摘要；PlainText 解析；Docling 适配器 | Docling；GROBID；Pilot 30 篇全文 | 章节、表格、引文、页码/边界框和高质量金标准 | A |
...
| WP4 多智能体证据校验 | 3 个业务 Agent；独立规则质量准入 | Qwen2.5-7B-Instruct；SciFact verifier；Debate-Augmented RAG 思路 | 单次抽取 vs. 命题生成/内部批判/质量准入消融、置信校准、成本 | D |
| WP5 应用与复现实验 | 原生 Web、Python HTTP 服务、14 项测试 | 统一实验 CLI；MLflow 或轻量 JSON registry；PySide6 APP | 数据冻结、实验复现、评审演示、失败恢复 | D 主责，全员集成 |
...
- 解析置信度进入关系质量评分，避免 OCR 错误被误判为科学事实。
...
当前固定 3 个业务 Agent。命题生成、批判、辩论和反证已归并为证据图谱 Agent 的内部策略，质量准入是独立的非 Agent 模块：
...
- 质量准入模块：优先使用验证集校准的逻辑回归/梯度提升或加权规则，不让同一个 LLM 自提、自批、自判；
...
| 普通 RAG + 单次抽取 | 检索增益 |
| 命题生成 + 质量准入 | 检查批判者贡献 |
| 命题生成 + 内部批判，无独立质量准入 | 检查质量准入贡献 |
| 三 Agent + 独立质量准入 | 完整系统 |
| 完整系统但打乱/删除证据 | 检查系统是否真正依赖证据 |
| 同模型生成与核验 vs. 独立模型/规则质量准入 | 检查角色同质化问题 |
...
- 维护“固定 3 Agent”口径；内部策略和质量模块不得包装成额外 Agent。

**近期任务**

1. 定义 `AgentMessage / AgentResult / EvidenceNote / Decision`。
2. 将证据图谱 Agent 的内部策略改为可替换 provider，并保持质量准入独立。
3. 实现 SciFact 验证器、Qwen 候选和可校准质量评分器。
...
- 三 Agent 轨迹完整率 100%，质量准入报告完整率 100%；
...
| 第 6–8 周 | 完整方法 | 解析分歧复核 | 全文/表格/证据模型 | 融合、冲突、社区 | 命题生成/内部批判/质量准入与校准 |
