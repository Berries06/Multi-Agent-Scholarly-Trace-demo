...
# 与结构抽取实验台的区别：本实验台额外接上「提出者 → 批判者 → 裁判 → 资源」
# 四个 Agent，让成员能看到一篇新论文从原文一路走到最终学习资源的每一步中间量。
#
# 运行环境：仓库统一 CPython 3.12 `.venv`。
# 启动（在项目根目录）：
#   $env:PYTHONPATH="src"
#   .venv\Scripts\python.exe -m streamlit run scripts/实验台/端到端流水线实验台.py --server.address 127.0.0.1 --server.port 8504
#
...
# 启动命令：python -m streamlit run scripts/实验台/端到端流水线实验台.py --server.address 127.0.0.1 --server.port 8504
