...
当前版本是可离线运行、可回归验证的 **Demo 工程基线**。已提供 3 个可切换垂直领域，每个领域 30 篇、合计 90 篇可核验 DOI/官方来源论文记录，并保留 19 篇已解析证据卡作为关系图谱核心；另有 9 组“领域 × 学习者”完整样例。系统真正跑通“论文证据卡 → 实体/关系抽取 → 可追溯图谱 → 三智能体裁决 → 技术脉络/Idea → 个性化资源”。新增的 71 篇元数据论文只参与书目检索，取得并解析全文前不能支撑知识关系；尚未完成大规模全文人工金标准、神经模型主实验和专家盲审。因此页面中的 24 条噪声压力集结果只用于暴露机制和失败案例，不能写成公开基准上的科研性能。
...
2. 双击根目录 `RUN_DEMO.bat`；
3. 等待浏览器自动打开 `http://127.0.0.1:8765/`；
4. 使用结束后双击 `STOP_DEMO.bat`。
