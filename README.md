...
执行检索规划、开放论文与官方技术文档多源召回、证据约束提出、批判裁决和资源生成。
...
- 实时路径并行检索官方文档、OpenAlex、Crossref 与 arXiv；配置
  `SEMANTIC_SCHOLAR_API_KEY` 后还会加入 Semantic Scholar。
- 实时路径只接受本轮召回的来源 ID，模型编造的来源会被确定性代码过滤。
- 单一数据源超时不会终止整轮检索；无关本地知识库不会被用作降级证据。
- 没有可靠证据或提出者返回零命题时，系统会明确拒答，不再返回 502。
...
- 单个开放数据源不可用时页面会列出警告并继续使用其他来源；所有外部来源不可用时，
  只有主题相关的本地切片才允许降级。
...
- 实时路径目前主要使用官方资料目录、论文元数据和摘要，不等于全文系统综述；
  重要结论仍需打开原始 URL、阅读全文或数据手册并人工复核。
